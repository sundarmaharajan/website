<?php
 ...
 if(isset($_POST['login']))
	  {
	$query ="select * from  truck where name='".$_POST['uid']."'  and	password ='".$_POST['password']."'";
	$result = mysql_query($query);
	if(mysql_num_rows($result))
		{
	$row = mysql_fetch_assoc($result);
		$_SESSION['login_user'] = $row['name'];
		echo '<script> alert("Login success"); window.location.href = "ownerhome.php" </script>';
		}
 else
	 {
...
<meta name="viewport" content="width=device-width, initial-scale=1">
  ...
  <table class="displaycontent" width="400" height="120">
  	<tr>
    <td><label for="uname"><p>Owner Name</p></label></td>
  <td> <input type="text" placeholder="Enter Username" name="uid" required class="textbox"></td>
   ...
   <td> <input type="password" placeholder="Enter Password" name="password" required class="textbox"></td>
	...
	<a href="ownregister.php" class="nav-item nav-link">New Register?</a>
